...
def check_starttime_vix_condition(time, vix_limit):
    ...
    if dt.datetime.now() > dt.datetime.strptime(dt.datetime.now().strftime('%Y-%m-%d') + time, '%Y-%m-%d %H:%M') and dt.datetime.now() < dt.datetime.strptime(dt.datetime.now().strftime('%Y-%m-%d')+' 23:00','%Y-%m-%d %H:%M'):
...
def main():
    ...
    print(f"Ticker EXPIRyyyyyy = {ticker_expiry}")
    ...
    #while False:
        ...
        def clear_excel_function(exl):
            #print("Which excel sheet to clear function for {exl.name}.......") 
            clear_excel = True
        ...
        if dt.datetime.now() > dt.datetime.strptime(dt.datetime.now().strftime('%Y-%m-%d')+' 15:45','%Y-%m-%d %H:%M'):
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Check Starategy Condition------------------------------------------------
        if expiry_bull_call_spread_startcondition1[1] is not None: # to save computation, we first check this
            if check_starttime_vix_condition(" 09:35", 12) == True and already_initialorder_done_for_bull_call_spread == False:
                print(colored("All conditions cleared for Expiry", "magenta"))
                
                already_initialorder_done_for_bull_call_spread = True # so that this will not run for second time
        ...
        if check_starttime_vix_condition(" 10:00", 12) == True and already_initialorder_done_for_delta_nutral == False:
            delta_nutral()
